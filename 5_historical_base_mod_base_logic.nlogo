...
to go
  ...
  if ticks >= 19584 [stop] ;;february 14, PBYs withdrawn to australia
...
to base_rehide
  ask seaplane_bases [
    if notFound = false and ticks mod (288 * 2) = 0 [
...
to seaplane_generate
  ...
  ask seaplane_bases [ hatch-seaplanes round(planes_sea / 2) [  ;;hatches half the seaplanes at the base- need to calibrate with historical sortie data
    ...
    set label-color green
    set fuel 120
    set sea_home_base one-of seaplane_bases-here
    ]
...
to seaplane_go
  ask seaplanes [
    ...
    face target-patch
      ...
      [set target-patch one-of seaplane_bases]
      if target-patch = nobody
      [stop]
    face target-patch
      seaplane_move
    if one-of seaplane_bases-here =  target-patch ; this block adds the seaplanes back to the base
...
to bomber_generate
  ...
  ask bases [ hatch-bombers (planes_land / 2) [
    ...
    set land_home_base one-of bases
    set fuel 120
    ]
...
to bomber_go
  ask bombers [
    ...
    face target-patch
      bomber_move
    if one-of bases-here = target-patch ; this block adds the seaplanes back to the base
...
ask red_bombers [
  ...
  red_bombers_move
    ...
    fd 1
    if patch-here = red_bomber_base ; this block adds the red bomber back to the base
          [
...
to tracking_helper [given_bases]
  ...
  [
    let dist_to_base distance one-of red_bases
...
@#$#@#$#@
<experiments>
  <experiment name="B-17 Test" repetitions="200" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>tonnage_B_17</metric>
    <enumeratedValueSet variable="PBY">
      <value value="45"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="PBY Test" repetitions="200" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>tonnage_PBY</metric>
    <enumeratedValueSet variable="PBY">
      <value value="45"/>
    </enumeratedValueSet>
  </experiment>
