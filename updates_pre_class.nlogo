...
to seaplane_generate
  ...
  ask seaplane_bases [ hatch-seaplanes round(planes_sea / 2) [  ;;quartered the number of seaplanes because the number of seaplanes was TOO DAMN HIGH, made it so bases overrun by land can't launch planes
    set color green
    set size 2.5  ;; easier to see
    set label-color green
    set fuel 100
...
to seaplane_go
  ask seaplanes [
    ...
    [
      let target-patch one-of (patches with [ (pycor > 133 and pycor < 142 and pxcor > 15 and pxcor < 60)]) ;sets target for mission to a red occupied patch
    face target-patch
